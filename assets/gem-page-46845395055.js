    ...
    jQuery(function() {
  var $hero = jQuery('#m-1604589009801');
  var $module = jQuery('#m-1604589009801').children('.module');
  ...
  if($module.attr('data-fixedMode') == '1'){
    ...
    $module.parent().removeAttr('style');
        ...
        jQuery(function() {
            var mode = jQuery('.gryffeditor').hasClass('editing') ? 'dev' : 'production';
            var $module = jQuery('#m-1574085636591').children('.module');
            if (mode == 'dev') {
                jQuery('#m-1574085636591').attr('data-name', '').css('background-image', 'none').css('min-height', '50px').removeAttr('data-image');
                
                var flag = true;
                var $bkLiquid = parent.jQuery('body').find('#gfFrame').contents().find('#module-1574085636591');
                if ($bkLiquid && $bkLiquid.length > 0) {
                    ...
                    try {
                        ...
                        if (imageUrl != '') {
                            flag = false;
                            jQuery('#m-1574085636591').css('background-image', 'url(' + imageUrl + ')').css('min-height', '100px').attr('data-image', 'true');
                        }
                        if (name != '' && name != 'Custom Code') {
                            flag = false;
                            jQuery('#m-1574085636591').attr('data-name', name);
                        }
                    } catch(error) {
                        console.log(error);
                    }
                ...
                if (flag) {
                    jQuery('#m-1574085636591').attr('data-name', 'Right click on the module, then choose Edit Html / Liquid option to start writing your custom code.');
                }
            }
        });
  
        jQuery(function() {
            var $module = jQuery('#m-1574085490120').children('.module');
            $module.gfV1Gmap();
    ...
    jQuery(function() {
        var $module = jQuery('#m-1574183629201').children('.module');
        if(jQuery().gfYoutube) {
            $module.gfYoutube();
        }
    });
  jQuery(function() {
  try {
    var $module = jQuery('#m-1579623573132').children('.module');   
    ...
    $module.owlCarousel({
      ...
      responsive:{
        ...
        jQuery(function() {
            var $module = jQuery('#m-1575469091596').children('.module');
        }); 
    
        jQuery(function() {
            var $module = jQuery('#m-1575469091628').children('.module');
        }); 
    
        jQuery(function() {
            var $module = jQuery('#m-1575469091621').children('.module');
        }); 
    
          jQuery(function() {
  var mode = jQuery('.gryffeditor').hasClass('editing') ? 'dev' : 'production';
  var $module = jQuery('#m-1598973683807').children('.module');
  if (mode == 'dev') {
      jQuery('#m-1598973683807').attr('data-name', '').css('background-image', 'none').removeAttr('data-image');
      
      var flag = true;
      var $bkLiquid = parent.jQuery('body').find('#gfFrame').contents().find('#module-1598973683807');
      if ($bkLiquid && $bkLiquid.length > 0) {
          var $settings = $bkLiquid.find('.settings');
          try {
              var name = '';
              ...
              if (imageUrl != '') {
                  flag = false;
                  jQuery('#m-1598973683807').css('background-image', 'url(' + imageUrl + ')').css('min-height', '100px').attr('data-image', 'true');
              }
              if (name != '' && name != 'Custom Code') {
                  flag = false;
                  jQuery('#m-1598973683807').attr('data-name', name);
              }
          } catch(error) {
              console.log(error);
          }
      ...
      if (flag) {
          jQuery('#m-1598973683807').attr('data-name', 'Right click on the module, then choose Edit Html / Liquid option to start writing your custom code.');
      }
  }
});
      
        jQuery(function() {
            var $module = jQuery('#m-1575469091532').children('.module');
